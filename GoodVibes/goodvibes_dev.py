...
#
# $Id: goodvibes_dev.py 106 2006-10-15 22:42:40Z tc $
#
...
##                    if dist_sq <= cutoff_sq:
    ...
    def eigenv_calccomb(self, matrix_hessian, jobid):
...
###########################
    ...
    def plot(self, jobid, biolunitchains, cutoff_distance, eigenvectors, chain, biomolecule, helices1, strands1, model, fileprefix, calctype, modes, ymax):
...
##            fd.close()
    ...
    def loop_axis_x(self, res1, jobid, winsize, datadic, fileprefix):

        filename = '%ssize%s_residue%s.txt' %(fileprefix, winsize, res1+1)
        lines = []
        for key in datadic:
...
##            sys.path.append('/var/www/cgi-bin/goodvibes/python/goodvibes/')
        ...
        matrix_hessian_perturbed = self.hessian_calculation(coordinates_cluster, float(cutoff_distance)) ## calculate with coordinates2 as well... and compare results to switching pdb1 and pdb2; actually a slower step than eigenvec calc... optimize!
        eigenvectors_perturbed, eigenvalues_perturbed, eigenvectors_perturbed_combined = self.eigenv_calccomb(matrix_hessian_perturbed, jobid)
...
##            chains_identical_1.append(self.identify_identical_chains_intrapdb(pdb1ATOM_all, chains1))
        ...
        for biomolecule in biomolecules: ##biomolecules = chains eller first chain if chains not given by user
            ...
            for cutoff_distance in cutoff_distances:
                ...
                eigenvectors_nonperturbed, eigenvalues_nonperturbed, eigenvectors_combined_nonperturbed = self.eigenv_calccomb(matrix_hessian, jobid)
                ...
                self.plot(jobid, biolunitchains, cutoff_distance, eigenvectors_nonperturbed, chain, biomolecule, helices1, strands1, pdb1model, fileprefix, calctype, 'single', ymax)
                ## do plots for combined modes
                self.plot(jobid, biolunitchains, cutoff_distance, eigenvectors_combined_nonperturbed, chain, biomolecule, helices1, strands1, pdb1model, fileprefix+'combo_', calctype, 'combo', ymax)
...
################################################################################
                ...
                for mode in range(3*residue_max):
                    if mode not in range(6,18)+[3*residue_max-1]:
                        ...
                        resres_eigenvalue_of_max_overlap.append(Numeric.zeros((residue_max,ymax),typecode='d'))
                        resres_mode_of_max_overlap.append(Numeric.zeros((residue_max,ymax),typecode='d'))
                        resres_eigenvalues_perturbed.append(Numeric.zeros((residue_max,ymax),typecode='d'))
                        resres_overlaps_single.append(Numeric.zeros((residue_max,ymax),typecode='d'))
                        resres_overlaps_combined.append(Numeric.zeros((residue_max,ymax),typecode='d'))
                datadic = {
...
################################################################################
                ...
                for winsize in winsizes:
                    ...
                    for remres1 in range(residue_max):
                        ...
                        if os.path.isfile('%ssize%s_residue%s.txt' %(fileprefix, winsize, remres1+1)):
                            ## read datafile to data variables before you continue
                            filename = '%ssize%s_residue%s.txt' %(fileprefix, winsize, remres1+1)
                            fd = open(filename, 'r')
                        ...
                        ## create data files (necessary to create data files if multithreading/parallel processing dependent on existence of files to determine if calculation on certain residues has been performed)
                            ...
                            fd = open('%ssize%s_residue%s.txt' %(fileprefix, winsize, remres1+1), 'w')
                            fd.close()
                        ...
                        self.loop_axis_x(
                            remres1, jobid, winsize, datadic, fileprefix)
...
################################################################################
                ...
                self.gnuplot_splot(
                    ...
                    ymax,
                    xtitle = axistitle, ytitle = ytitle, ztitle = '{/Symbol D}overlap',
                ...
                for mode in range(6,12):
                    ...
                    for key in datadic:
                        ...
                        self.gnuplot_splot(
                            ...
                            ymax,
                            xtitle = axistitle, ytitle = ytitle, ztitle = datadic[key]['name'],
...
##        print 'done'
    ...
    def gnuplot_plot(self, jobid, cutoff_distance, chains, title1, xtitle, ytitle, plotname, filename, data):
        ...
        os.system('gnuplot %s.gnuplotsettings' %(filename))
