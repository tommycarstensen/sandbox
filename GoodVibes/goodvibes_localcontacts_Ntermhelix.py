...
class vibration:
    ...
    def morph(self, eigenvectors_all_modes, nframes, chains, d_coordinates, atoms_hessian, job,):
...
##            eigenvectors = length_adjustment(mode, eigenvectors)
    ...
    def main(
        ...
        self.morph(eigenvectors_nonperturbed, frames, chains, d_coordinates, atoms_hessian, jobid,)
...
##            )
        ...
        resranges = [
            [range(0,13),range(79,164)],
            [range(0,59),range(79,164)],
            ]
        cluster = []

        for i in range(len(resranges)):

            resrange1 = resranges[i][0]
            resrange2 = resranges[i][1]

            matrix_hessian_perturbed = self.hessian_calculation(
                l_coordinates, float(cutoff_distance), verbose,
                resrange1 = resrange1, resrange2 = resrange2,
                )
            (
                eigenvectors_perturbed, eigenvalues_perturbed, eigenvectors_perturbed_combined
                ) = self.eigenv_calccomb(
            ...
            (
                ...
                ) = self.overlap_calculation(
                    eigenvectors_perturbed, eigenvectors_nonperturbed, cluster, eigenvalues_perturbed, eigenvalues_nonperturbed
                    )
            print len(overlaps_single), overlaps_single[:12]
            overlaps_combined, max_overlaps_combined, perturbed_modes_of_max_overlap_combined = self.overlap_calculation(
                eigenvectors_perturbed_combined, eigenvectors_nonperturbed, cluster, eigenvalues_perturbed, eigenvalues_nonperturbed
                )[:-1]
            self.morph(
                eigenvectors_perturbed, frames, chains, d_coordinates, atoms_hessian, jobid+str(i),
                )
...
##        print 'parsing info from coordinate section about residues, atoms and coordinates'
    ...
    def hessian_calculation(self, coordinates, cutoff, verbose = False, resrange1 = [], resrange2 = []):
