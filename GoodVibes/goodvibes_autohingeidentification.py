...
#
# $Id: goodvibes_autohingeidentification.py 27 2006-06-27 15:35:21Z tc $
#
...
possible.'''
        ...
        angle = float(job[job.find('_angle')+6:job.find('_depth')])
        depth = float(job[job.find('_depth')+6:job.find('.pdb')])
        bandheight = 3.5/math.sin(math.radians(angle/2))
        normalx = -math.sin(math.pi/2-math.radians(angle/2))
        normaly = math.cos(math.pi/2-math.radians(angle/2))
        print 'angle%s, depth%s, normalx%s, normaly%s' %(angle, depth, normalx, normaly)
                ...
                for residue in pdb1ATOM_all[chain1]['residues'].keys():
                    ...
                    for atom in pdb1ATOM_all[chain1]['residues'][residue]['atoms'].keys():
                        if atom in atoms_hessian: ## skriv den latterlige key morph til pdb1coords dic'en!!!!!!!!!!!!
                            ...
                            xs = coordinate[0]
                            ys = coordinate[1]
                            R = math.pow(500. * 100. / ((4./3)*math.pi), 1./3)
                            if ys > R-depth and xs < (ys-(R-depth-bandheight))*math.tan(math.pi*angle/360.) and xs > -(ys-(R-depth-bandheight))*math.tan(math.pi*angle/360.):
                                if mode == 6:
                                    if xs > 0:
                                        right[0] += eigenvectors[i+0]
                                        right[1] += eigenvectors[i+1]
                                        right[2] += eigenvectors[i+2]
                                    if xs < 0:
                                        left[0] += eigenvectors[i+0]
                                        left[1] += eigenvectors[i+1]
                                        left[2] += eigenvectors[i+2]
                            i += 3

            if mode == 6:

                if max(pdb1ATOM_all['A']['residues']) > 250 and left != [0,0,0] and right != [0,0,0]:
                    ## psi is the angle between the vectors using only the x and y composants
                    psi = (
                        180*math.acos((right[0]*normalx+right[1]*normaly)/math.sqrt((right[0]**2+right[1]**2)*(normalx**2+normaly**2)))/math.pi
                        +
                        180*math.acos((left[0]*(-normalx)+left[1]*normaly)/math.sqrt((left[0]**2+left[1]**2)*(normalx**2+normaly**2)))/math.pi
                        )/2
                    ## omega is the angle between the vectors using all composants
                    omega = (
                        180*math.acos((right[0]*normalx+right[1]*normaly)/math.sqrt((right[0]**2+right[1]**2+right[2]**2)*(normalx**2+normaly**2)))/math.pi
                        +
                        180*math.acos((left[0]*(-normalx)+left[1]*normaly)/math.sqrt((left[0]**2+left[1]**2+left[2]**2)*(normalx**2+normaly**2)))/math.pi
                        )/2
                    if right[0] > 0 and left[0] < 0:
                        omega -= 180
                        psi -= 180
                    centralangle = math.pi-math.asin((R-depth)*math.sin(math.pi-(math.pi*angle/180)/2)/R)-(math.pi*angle/180)/2
                    chord = 2*R*math.sin(centralangle)
                    print chord
                    print 'omega', omega, 'psi', psi
                    print 'leftright', left, right
                    print 'normalxy', normalx, normaly
                    fd = open('angles.txt', 'a')
                    fd.write('%3i %2i %4i %4i %2i \n' %(angle, depth, omega, psi, cutoff))
                    fd.close()
                    if (  ( abs(left[0]) < abs(left[1]) or abs(right[0]) < abs(right[1]) ) or ( abs(left[0]) < abs(left[2]) or abs(left[0]) < abs(left[2]) )  ) or left[0]/right[0] > 0 or max(pdb1ATOM_all['A']['residues']) < 100:
                        print xs, ys, coordinate[2], residue
                        print left, right
                        fd = open('nohinge.txt', 'a')
                        fd.write('%s %s \n' %(angle, depth))
                        fd.close()
...
##                                    fd.write(str(clusterplotdata[mode_clustersize0-6][mode-6][clustersize-1][residue_cluster_central])+'\n')
                    ...
                    del eigenvectors_clustersize0 ## necessary due to ## 3) clustersize == 0 if not vars().has_key('eigenvectors_clustersize0'):
