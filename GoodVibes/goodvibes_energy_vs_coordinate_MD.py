...
##                os.remove('frame%s.pdb' %(str(frame).zfill(2)))
    ...
    def main(
        ...
        for MDframe in range(101):
            pdb2 = '2LZM_wt_trjconv_%i' %(MDframe)
            chains2 = [' ']

            fd = open('%s.pdb' %(pdb2),'r')
            lines2 = fd.readlines()
            fd.close()
            ## parse pdb2
            (
                ...
                d_ligands2,
                ) = goodvibes_core.parse_pdb(lines2, chains2, parse_remarks = False)
    ...
    ##                        E -= .5*kx*((vxb-vxa)**2)+.5*ky*((vyb-vya)**2)+.5*kz*((vzb-vza)**2)
            ...
            for E in l_E:
                l += ['%s\n' %(E)]
            fd = open('energies%s.txt' %(MDframe),'w')
            fd.writelines(l)
            fd.close()
    ...
    ##                    'ribbons\n',
            ...
            ## convert rasmol output
            os.system('convert %s.ppm %s.gif' %(pdb2,pdb2,))
            ## clean up
            os.remove('%s.ppm' %(pdb2))
            os.remove('rasmol.log')
    ##        os.remove('rasmol.src')

            print MDframe,'minmax', mini,maxi
...
##            )
        ...
        ## merge gifs to animation
        line = 'convert '
        for i in range(101)+range(101-1-1,-1+1,-1): ## 0 to 100 and 99 to 1
            line += '2LZM_wt_trjconv_%i.gif ' %(i)
