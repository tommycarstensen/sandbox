...
##                os.remove('frame%s.pdb' %(str(frame).zfill(2)))
    ...
    def main(
        ...
        d_energies = {}
        d_factors = {}
        for mode in range(6,min(12,3*N)):
            print 'calculating energies of mode', mode
            d_energies[mode] = {}
...
##                print f,E
            ...
            for i in range(0,len(matrix_hessian),3):
                ...
                d_energies[mode][res_index1] = {}
                ## loop over frames
                for frame in range(51):

                    vxa = 20*eigenvectors_nonperturbed[mode][i+0]*((25-frame)/25.)
                    vya = 20*eigenvectors_nonperturbed[mode][i+1]*((25-frame)/25.)
                    vza = 20*eigenvectors_nonperturbed[mode][i+2]*((25-frame)/25.)

                    E = 0
##                    if frame == 0 and res_index1 == 33 and mode == 6:
##                        l_E = []
                    for j in range(0,len(matrix_hessian),3):
                        ...
                        vxb = 20*eigenvectors_nonperturbed[mode][j+0]*((25-frame)/25.)
                        vyb = 20*eigenvectors_nonperturbed[mode][j+1]*((25-frame)/25.)
                        vzb = 20*eigenvectors_nonperturbed[mode][j+2]*((25-frame)/25.)
...
##                        if frame == 0 and res_index1 == 33 and mode == 6:
##                            l_E += [[.5*kx*((vxb-vxa)**2)+.5*ky*((vyb-vya)**2)+.5*kz*((vzb-vza)**2),res_index2+1]]

                    if frame == 0:
                        l_E += [E]

                    d_energies[mode][res_index1][frame] = E
##                    if res_index1 == 33 and mode == 6:
##                        print E
                    if E < mini:
                        mini = E
                    if E > maxi[0]:
            ...
            ## write energies to txt file for comparison to nma
            l = []
            for E in l_E:
                l += ['%s\n' %(E)]
            fd = open('energies%s.txt' %(mode),'w')
            fd.writelines(l)
            fd.close()
