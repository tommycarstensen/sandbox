...
##            '1run',
            ...
            '2c03','1ljy','1fzg','1n2c','1r9m','2bod','1p8h','2fa7','2cfg','2bhy','1sbd','1sbe','1ppv','1x83','2v5s','3car','1ea5','2c3w',
...
##            '1t7b','1t7e','1tud','1yra','2qwa','2qwb','2qwc','2qwd','2qwe','2qwf','2qwg','2qwh','2qwi','2qwj','2qwk',
    ...
    ######### manual selection
            ...
            d_rmsd = self.analyze_sequences(d_seq, d_chains_intrapdb_sequence_identical,verbose=True)
...
##                    print '***', file
    ...
    ##            os.remove('/oxygenase_local/tc/quakes/htm/%s.htm' %(pdb))
            ...
            for set_pdb in sets_pdbs:
                ...
                self.pdbcount = len(self.l_pdbs)
                d_seq, d_chains_intrapdb_sequence_identical = self.parse_sequences()
                d_rmsd = self.analyze_sequences(d_seq, d_chains_intrapdb_sequence_identical,verbose=False)
                self.write_rmsd_to_file(d_rmsd, d_seq, prefix='rmsd')
##############
...
##                                '1fgn',
    ...
    def analyze_sequences(self, d_seq, d_chains_intrapdb_sequence_identical,verbose=False):
                ...
                for biomolecule1 in d_biomolecules1.keys():
                    ...
                    for biomolecule2 in d_biomolecules2.keys():
                            ...
                            for pdb in d_bmSEQRESchains_not_similar_to_SEQRESchains.keys():
                                if len(d_bmSEQRESchains_not_similar_to_SEQRESchains[pdb]['chains1']) > 0:
                                    for chain in d_bmSEQRESchains_not_similar_to_SEQRESchains[pdb]['chains1']:
                                        ...
                                        long = False
                                        ## check if peptide
                                        if d_seq[pdb]['SEQRES'][chain]['type'] == 'peptide':
                                            peptide = True
                                        ## check if long chain
                                        if len(d_seq[pdb]['SEQRES'][chain]['seq']) > self.min_len_chain:
                                            long = True
                        ...
                        if pdb1 not in d_pdb.keys():
                            d_pdb,d_hetero = self.parse_coordinates(pdb1, d_pdb, d_seq[pdb1],d_hetero, verbose = verbose)
                        d_pdb,d_hetero = self.parse_coordinates(pdb2, d_pdb, d_seq[pdb2],d_hetero,verbose=verbose)

                        ## skip if only alpha carbon atoms (e.g. 1thi)
                        alpha1 = self.identify_atom_types_in_long_peptide_chains(d_seq[pdb1], d_pdb[pdb1])
                        if alpha1 == True: 
                            print 'alpha', pdb1
                            continue
                        alpha2 = self.identify_atom_types_in_long_peptide_chains(d_seq[pdb2], d_pdb[pdb2])
                        if alpha2 == True:
                            print 'alpha', pdb2
                            continue
...
##                                )
                        ...
                        ##
                        n_mutations,d_mutations = self.identify_mutations(pdb1, pdb2, l_equivalent_chains, d_chains_interpdb_sequence_similar, d_chains_intrapdb_sequence_identical)


                        if n_mutations == 1 and pdb1 not in ['1nvv',]:
                            d_loop = {
                            ...
                            ## get residue index dictionary of mutated chain
                            dontgetdresnos_twice
                            d_res_nos1, d_res_nos2, l1, l2, ATOMseq1, ATOMseq2 = self.alignATOMseq(
                                d_pdb, d_seq, pdb1, pdb2, chain1, chain2,
                                )
                            d_loop[pdb1]['d_res_nos'] = d_res_nos1
                            d_loop[pdb2]['d_res_nos'] = d_res_nos2
                            for pdb in d_loop:
                                chain = d_loop[pdb]['chain']
                                ##
                                d_res_nos = d_loop[pdb]['d_res_nos']
                                ## get residue number of mutation
                                res_no = d_res_nos[res_index]['res_no']
                                ## get insertion code of mutation
                                iCode = d_res_nos[res_index]['iCode']
                                ## get coordinates
                                if d_pdb[pdb]['chains'][chain]['residues'][res_no]['d_iCodes'][iCode]['record'] == 'REMARK465':
                                    phi = '   N/A'
                                    psi = '   N/A'
                                elif (
                                    ...
                                    ## calculate dihedrals
                                    if res_index-1 in d_res_nos.keys():
                                        res_no_prev = d_res_nos[res_index-1]['res_no']
                                        iCode_prev = d_res_nos[res_index-1]['iCode']
                                        ...
                                        phi = '   N/A'
                                    if res_index+1 in d_res_nos.keys():
                                        res_no_next = d_res_nos[res_index+1]['res_no']
                                        iCode_next = d_res_nos[res_index+1]['iCode']
...
##                        d_pdb[pdb1] = self.whatif(pdb1,biomolecule1,d_pdb[pdb1])
    ...
    def compare_hetero_compounds(
        ...
        for pdbpair in pdbpairs:
            ...
            for roota in d_hetero[pdba].keys():
                ...
                if roota[0] in d_seq[pdba]['chains'].keys() and d_seq[pdba]['chains'][roota[0]]['type'] == 'nucleotide' and len(d_seq[pdba]['chains'][roota[0]]['seq']) <= 3:
                    continue
                ...
                for rootb in d_hetero[pdbb].keys():
                    ...
                    ## continue if root is a short nucleotide (to avoid reversly numbered nucleotides etc.)
                    if rootb[0] in d_seq[pdbb]['chains'].keys() and d_seq[pdbb]['chains'][rootb[0]]['type'] == 'nucleotide' and len(d_seq[pdbb]['chains'][rootb[0]]['seq']) <= 3:
                        continue
                    ...
                    if compounda == compoundb:
                        ...
                        print d_seq[pdba]['chains'][roota[0]]
                        print d_seq[pdbb]['chains'][rootb[0]]
                        ...
                        print '*************'
                        dontgetdresnostwice
                        d_res_nosa, d_res_nosb, l1, l2, ATOMseq1, ATOMseq2 = self.alignATOMseq(d_pdb, d_seq, pdba, pdbb, roota[0], rootb[0])
                        ...
                        if ka+l1 != kb+l2: ## e.g. 2jbj,2pvw;1z4v,1z4w,1z4z;1d6q,1re2;1o8a,2iux
                            print ka,l1
                            print kb,l2
                            print pdba,roota,rootsa,d_res_nosa[ka],d_seq[pdba]['chains'][roota[0]]['seq'][ka]
                            print pdbb,rootb,rootsb,d_res_nosb[kb],d_seq[pdbb]['chains'][rootb[0]]['seq'][kb]
                            print self.cluster, 'cluster'
                            identical = False
                            continue
                        if d_seq[pdba]['chains'][roota[0]]['seq'][ka] != d_seq[pdbb]['chains'][rootb[0]]['seq'][kb]:
                            stop_expected3
            ...
            if different == True:
                ...
                fd.write('%s %s\n%s\n%s\n' %(pdb1, pdb2, d_seq[pdb1]['TITLE'], d_seq[pdb2]['TITLE']))
                fd.close()
                break
    ...
    def identify_atom_types_in_long_peptide_chains(self, d_seq, d_pdb):

        atoms = set()
        for chain in d_seq['SEQRES'].keys():
            if d_seq['SEQRES'][chain]['type'] != 'peptide' or len(d_seq['SEQRES'][chain]['seq']) < self.min_len_chain:
                continue
...
##        print 'identifying different polymers (not long peptides)'
        ...
        for pdb in d_chains.keys():
            for chain in d_seq[pdb]['SEQRES']:
                chain_type = d_seq[pdb]['SEQRES'][chain]['type']
                chain_seq = d_seq[pdb]['SEQRES'][chain]['seq']
    ...
    def different_hetero_compounds(self,pdb1,pdb2,d_seq):
        ...
        for pdb in d_hetIDs:
            for chain in d_seq[pdb]['HET'].keys():
                for res_no in d_seq[pdb]['HET'][chain].keys():
                    for iCode in d_seq[pdb]['HET'][chain][res_no].keys():
                        d_hetIDs[pdb] |= set([d_seq[pdb]['HET'][chain][res_no][iCode]])
...
## change stereoisomers earlier and skip this step!!!
    ...
    def pdbskip(self, d_seq, pdb):
        ...
        pdbskip = False
        SEQRESchains = d_seq[pdb]['SEQRES'].keys()
...
##                return pdbskip
    ...
    def rmsd2bfactor(
        self, pdb1, pdb2, biomolecule1, biomolecule2, rmsd, d_pdb, d_seq,
        tv1, rm, tv2,
        ...
        for i in range(len(chains1)):
            ...
            coordinates1, coordinates2, rescount, lines1, lines2 = self.ATOMrecords2coordinates(
                d_pdb, pdb1, pdb2, chain1, chain2, d_res_nos1, d_res_nos2, l1, l2, len(ATOMseq1),
    ...
    def write_rmsd_to_file(self, d_rmsd, d_seq, prefix):
        ...
        for pdb1 in d_rmsd:
            ...
            for chain in d_seq[pdb1]['HET'].keys():
                for res_no in d_seq[pdb1]['HET'][chain].keys():
                    for iCode in d_seq[pdb1]['HET'][chain][res_no].keys():
                        hetIDs1 |= set([d_seq[pdb1]['HET'][chain][res_no][iCode]])
            ...
            for bm1 in d_rmsd[pdb1].keys():
                ...
                for pdb2 in d_rmsd[pdb1][bm1]:
                    ...
                    for chain in d_seq[pdb2]['HET'].keys():
                        for res_no in d_seq[pdb2]['HET'][chain].keys():
                            for iCode in d_seq[pdb2]['HET'][chain][res_no].keys():
                                hetIDs2 |= set([d_seq[pdb2]['HET'][chain][res_no][iCode]])
...
##        print 'identifying different long peptides'
        ...
        SEQRESchains1_not_similar_to_SEQRESchains2 = set(d_seq[pdb1]['SEQRES'].keys()) & (set(bmchains1) - SEQRESchains1_similar_to_SEQRESchains2)
        SEQRESchains2_not_similar_to_SEQRESchains1 = set(d_seq[pdb2]['SEQRES'].keys()) & (set(bmchains2) - SEQRESchains2_similar_to_SEQRESchains1)
...
##        print 'calculating rmsd for chains %s, %s of %s, %s' %(chains1, chains2, pdb1, pdb2)
        ...
        ## align residues for the specific combinations of chain1 and chain2
        ##
        d_seqalnparam = self.align_residues(chains1, chains2, d_pdb, d_seq, pdb1, pdb2, d_chains_interpdb_sequence_similar)
        ...
        for i in range(len(chains1)):
            ...
            dontcalcdresnostwice
            d_res_nos1 = d_chains_interpdb_sequence_similar[chain1[0]][chain2[0]]['d_res_nos1']
            d_res_nos2 = d_chains_interpdb_sequence_similar[chain1[0]][chain2[0]]['d_res_nos2']
            l1 = d_seqalnparam[chain1[0]][chain2[0]]['l1']
            l2 = d_seqalnparam[chain1[0]][chain2[0]]['l2']
            ATOMseq1 = d_seqalnparam[chain1[0]][chain2[0]]['seqATOM1']
            ATOMseq2 = d_seqalnparam[chain1[0]][chain2[0]]['seqATOM2']
    ...
    def align_residues(self, chains1, chains2, d_pdb, d_seq, pdb1, pdb2, d_chains_interpdb_sequence_similar,):
...
##                    continue
        ...
        ## incorrect transformation and 1 chain v 1 chain
            ...
            if rmsd > self.maxrmsd_wrong:
                self.incorrecttransformation(d_seq,pdb1,pdb2,biomolecule1,biomolecule2,chains1,chains2,rmsd,)
            l_equivalent_chains = [tchains1,tchains2]
            return l_equivalent_chains, rmsd, n_chains, n_residues, n_coordinates, tv1, rm, tv2, transformations
...
##                            tchains1_PISA,tchains2_PISA,d_pdb,pdb1,pdb2,d_seq,d_chains_interpdb_sequence_similar,d_chains_intrapdb_sequence_identical,verbose=True
##                            )
##
##                        ##
##                        ## check if the expected correct combination of chains gives a low rmsd
...
##                            return l_equivalent_chains, rmsd, n_chains, n_residues, n_coordinates, tv1, rm, tv2, transformations
            ...
            ##
            print 'different chain combinations of remark350 transformations'
            (
                l_equivalent_chains, rmsd, n_chains, n_residues, n_coordinates, tv1, rm, tv2,
                ) = self.shuffle_chains_and_calculate_rmsd(
                    d_pdb,d_seq,d_chains_interpdb_sequence_similar,d_chains_intrapdb_sequence_identical,
            ...
            if rmsd > self.maxrmsd_wrong:
                ...
                (
                    l_equivalent_chains, rmsd, n_chains, n_residues, n_coordinates, tv1, rm, tv2,
                        ...
                        d_pdb,pdb1,pdb2,biomolecule1,biomolecule2,chains1,chains2,d_seq,rmsd,
    ...
    def shuffle_chains_and_calculate_rmsd(
        self,d_pdb,d_seq,d_chains_interpdb_sequence_similar,d_chains_intrapdb_sequence_identical,
        pdb1,pdb2,l_tchains1,l_tchains2,
        ):
...
##                return d_pdb, tchains
    ...
    def remark290combinations(
        self,d_pdb,pdb1,pdb2,biomolecule1,biomolecule2,chains1,chains2,d_seq,rmsd,
        d_chains_interpdb_sequence_similar,d_chains_intrapdb_sequence_identical,
        ):
...
##        chains1 = d_seq[pdb1]['REMARK350'][biomolecule1]['chains'].keys()
##        chains2 = d_seq[pdb2]['REMARK350'][biomolecule2]['chains'].keys()
        operators1 = d_seq[pdb1]['REMARK290'].keys()
        operators2 = d_seq[pdb2]['REMARK290'].keys()

        if 'REMARK350' not in d_seq[pdb1].keys() and 'REMARK350' not in d_seq[pdb2].keys():
            ...
            if 'REMARK350' not in d_seq[pdb1].keys():
                if len(operators1) == 1:
                    n_matrices1 = 1
                else:
                    n_matrices1 = len(chains1)
            else:
                n_matrices1 = len(d_seq[pdb1]['REMARK350'][biomolecule1]['matrices'].keys())
            if 'REMARK350' not in d_seq[pdb2].keys():
                if len(operators2) == 1:
                    n_matrices2 = 1
                else:
                    n_matrices2 = len(chains2)
            else:
                n_matrices2 = len(d_seq[pdb2]['REMARK350'][biomolecule2]['matrices'].keys())
...
##            set([pdb1,pdb2]) != set(['1r4c','1tij'])
            ...
            print d_seq[pdb1].keys()
            print d_seq[pdb2].keys()
##            stop_different_sized_multimers_check_remark350records
        ...
        if len(chains1) != len(chains2):
            if (
                ...
                len(chains1)*len(d_seq[pdb1]['REMARK350'][biomolecule1]['matrices'].keys()) != len(chains2)*len(d_seq[pdb2]['REMARK350'][biomolecule2]['matrices'].keys())
                ):
                print chains1, chains2
        ...
        for operator_combination1 in operator_combinations1:
            ...
            for chain1 in chains1:
                ...
                for operator1 in operator_combination1:
                    matrix1 = d_seq[pdb1]['REMARK290'][operator1]
            ...
            for operator_combination2 in operator_combinations2:
                ...
                for chain2 in chains2:
                    ...
                    for operator2 in operator_combination2:
                        matrix2 = d_seq[pdb2]['REMARK290'][operator2]
...
##                    print operator_combinations2
                    if (
                        ...
                        print pdb1, pdb2
                        print d_seq[pdb1]['REMARK350'][biomolecule1]['matrices'].keys()
                        print d_seq[pdb2]['REMARK350'][biomolecule2]['matrices'].keys()
                        print self.cluster, 'cluster'
                        notexpected
                    else:
                        continue
                print tchains1, tchains2
                rmsd, n_chains, n_residues, n_coordinates, tv1, rm, tv2 = self.calculate_rmsd_for_multiple_chains(
                    tchains1,tchains2,d_pdb,pdb1,pdb2,d_seq,d_chains_interpdb_sequence_similar,d_chains_intrapdb_sequence_identical,verbose=True
                    )
                if rmsd < self.maxrmsd_wrong:
                    fd = open('symop.txt','a')
    ...
    def incorrecttransformation(self,d_seq,pdb1,pdb2,biomolecule1,biomolecule2,chains1,chains2,rmsd,):
        ...
        if not [pdb1,pdb2,biomolecule1,biomolecule2] in toomany and not [pdb2,pdb1,biomolecule2,biomolecule1] in toomany:
            ...
            fd.write('%s %s %s %s %3i   %5.1f %5s %10s %10s %s\n' %(pdb1, pdb2, biomolecule1, biomolecule2, len(chains1), rmsd, d_seq[pdb1]['CRYST1']==d_seq[pdb2]['CRYST1'], d_seq[pdb1]['CRYST1'], d_seq[pdb2]['CRYST1'], len(chains1)))
            fd.close()
...
##        print 'identify_identical_chains_from_sequence', pdb1, pdb2
        ...
        for i in range(len(chains)):
            ...
            seqi = d_seq[pdb]['SEQRES'][chaini]['seq']
            ...
            for j in range(i+1,len(chains)):
                ...
                seqj = d_seq[pdb]['SEQRES'][chainj]['seq']
...
##        print 'identifying similar chains'
        ...
        for chain1 in repchains1:
            ...
            if d_seq[pdb1]['SEQRES'][chain1]['type'] != 'peptide':
                continue

            seq1 = d_seq[pdb1]['SEQRES'][chain1]['seq']
            ...
            for chain2 in repchains2:
                ...
                if d_seq[pdb2]['SEQRES'][chain2]['type'] != 'peptide':
                    continue

                seq2 = d_seq[pdb2]['SEQRES'][chain2]['seq']
...
##        print d_chains_interpdb_sequence_similar
    ...
    def ATOM2seq(self, d_pdb, chain, d_seq, res_no_max = None, stop_error = True):
                    ...
                    if res_no in d_seq['HET'][chain].keys():
                        if iCode in d_seq['HET'][chain][res_no].keys():
                            if res_name != d_seq['HET'][chain][res_no][iCode]:
                                if stop_error == True:
                                    print chain, res_no, iCode, res_name, d_seq['HET'][chain][res_no][iCode], d_seq['MODRES'][chain][res_no][iCode]
                                    notexpected
                            else:
    ...
    def determine_if_modres(self, d_seq, d_pdb, chain, res_no, iCode, res_name):

        modres = False
        if chain in d_seq['MODRES'].keys():
            if res_no in d_seq['MODRES'][chain].keys():
                if iCode in d_seq['MODRES'][chain][res_no].keys():
                    if d_pdb['chains'][chain]['residues'][res_no]['d_iCodes'][iCode]['res_name'] == d_seq['MODRES'][chain][res_no][iCode]:
                        modres = True
                    else:
...
##        print SEQRESseq
    ...
    def alignATOMseq(self, d_pdb, d_seq, pdb1, pdb2, chain1, chain2):
        ...
        for pdb in d_ATOM_seqs.keys():
            ...
            SEQRESseq = d_seq[pdb]['SEQRES'][chain]['seq']
...
##                fd.close()
            ...
            if rmsd:
                ...
                for atom_name in d_atoms2.keys():
                    ...
                    coordinate2 = Numeric.matrixmultiply(coordinate2-tv1,rm)+tv2
...
##                fd.close()
    ...
    def parse_coordinates(self, s_pdb, d_pdb, d_seq, d_hetero, verbose = False):
...
##        fd = open('%s%s.pdb' %(self.path_pdb, s_pdb.upper()),'r')
        ...
        if s_pdb not in d_pdb.keys():
            d_coordinates,d_heteromolecules = self.parse_pdbcoordinatesection(lines, d_seq, verbose = verbose, s_pdb = s_pdb)
            d_pdb[s_pdb] = d_coordinates
            d_hetero[s_pdb] = d_heteromolecules
...
##            fd = open('%s%s.pdb' %(self.path_pdb, s_pdb.upper()),'r')
    ...
    def parse_pdbheader(self, lines, s_pdb = None, stop_error = True):
...
##                d_conect = self.parse_atom_no_range(d_conect, 'atom', atom_no)
                ...
                elif d_seq['HET'][chain][res_no][iCode] != hetID:
                    if stop_error == True:
                        print d_seq['HET'][chain][res_no][iCode], hetID
...
##                    print self.pdb, biounit
        ...
        saccharidechains = []
        for chain in d_seq['SEQRES'].keys():
            if d_seq['SEQRES'][chain]['type'] == 'peptide':
                peptidechains += chain
                if len(d_seq['SEQRES'][chain]['seq']) > self.min_len_chain:
                    proteinchains += chain
            elif d_seq['SEQRES'][chain]['type'] == 'nucleotide':
                nucleotidechains += chain
            elif d_seq['SEQRES'][chain]['type'] == 'saccharide':
                saccharidechains += chain

        d_seq['proteinchains'] = proteinchains

##        if s_pdb not in ['1ady','1bhj']:
##            chains = d_seq['SEQRES'].keys()
##            if 'REMARK350' not in d_seq.keys() and len(d_seq['SEQRES'].keys()) > 1 and d_seq['EXPDTA'] != 'NMR' and len(saccharidechains) != len(d_seq['SEQRES'].keys()):
...
#### count number of similar chains by comparing SEQRESseqs
    ...
    def parse_recordSEQRES(self, line, d_seq):
        ...
        if 'SEQRES' not in d_seq:
            d_seq['SEQRES'] = {}
        if chain not in d_seq['SEQRES'].keys():
            d_seq['SEQRES'][chain] = {}
        if not 'type' in d_seq['SEQRES'][chain].keys():
            d_seq['SEQRES'][chain]['type'] = 'N/A'
        ...
        for i in range(len(residues)):
            ...
            if residue in self.d_res.keys():
                if d_seq['SEQRES'][chain]['type'] == 'N/A':
                    d_seq['SEQRES'][chain]['type'] = 'peptide'
                residues[i] = self.d_res[residue]
            elif residue in self.l_nucleotides:
                if d_seq['SEQRES'][chain]['type'] == 'N/A':
                    d_seq['SEQRES'][chain]['type'] = 'nucleotide'
                residues[i] = residue
            elif residue in ['GLC','GAL','MAN','FRU']:
                if d_seq['SEQRES'][chain]['type'] == 'N/A':
                    d_seq['SEQRES'][chain]['type'] = 'saccharide'
                residues[i] = residue
            else:
        ...
        if 'seq' not in d_seq['SEQRES'][chain].keys():
            d_seq['SEQRES'][chain]['seq'] = ''
        d_seq['SEQRES'][chain]['seq'] += ''.join(residues)

        return d_seq


    def parse_pdbcoordinatesection(self, lines, d_seq, verbose = False, s_pdb = None, parse_molecules = True):
        ...
        for i in range(len(lines)):
            ...
            if record == 'ATOM':
                d_coordinates, d_line = self.parse_recordATOM(line, d_coordinates, lines, i, d_seq, 'ATOM')
                ...
                if res_name in ['D2O','H2O',]:
                    ...
                    stop
                ## parse water if connection to it (e.g. 2bpb)
                if res_name in ['HOH','DOD',]: ## DOD in 2d4j
                    d_coordinates, d_line = self.parse_recordATOM(line, d_coordinates, lines, i, d_seq, 'HETATM')
...
##                d_hetero['chains'][chain]['residues'][res_no]['iCodes'][iCode] = hetID
    ...
    ##                                    continue
                                    ...
                                    modres1 = self.determine_if_modres(d_seq, d_coordinates, chain1, res_no1, iCode1, hetID1)
                                    if modres1 == True:
                                        continue
                                    modres2 = self.determine_if_modres(d_seq, d_coordinates, chain2, res_no2, iCode2, hetID2)
                                    if modres2 == True:
                                        continue
    ...
    ##        print d_molecules
        ...
        return d_coordinates, d_molecules
    ...
    def parse_recordREMARK(self, d_seq, line, i, lines):
        ...
        if remark == 200:
            ...
            d_seq = self.parse_recordREMARK290(d_seq, i, lines)
    ...
    def parse_recordREMARK290(self, d_seq, i, lines):

        line = lines[i]

        if 'REMARK290' not in d_seq.keys():
            d_seq['REMARK290'] = {}
        ...
        if line[13:18] == 'SMTRY':
            ...
            if dimension == 1:
                ...
                d_seq['REMARK290'][operator] = [matrixrow1,matrixrow2,matrixrow3,]
...
##                        transformation = True
    ...
    def parse_recordREMARK465(self, line, d_pdb, lines, i):
        ...
        if line[10:].strip() in ['M RES C SSSEQI','M RES C  SSEQI']:
            ...
            for j in range(i+1,len(lines)):
                ...
                ## iCode > record
                d_pdb['chains'][chain]['residues'][res_no]['d_iCodes'][iCode]['record'] = 'REMARK465'
    ...
    def parse_recordATOM(self, line, d_pdb, lines, i, d_seq, record):
...
##            stop_duplicate_remark465_and_coord_section
        ...
        if res_name_conflict == False:
            ...
            if len(d_pdb['chains'][chain]['residues'][res_no]['l_iCodes']) == 0:
                ...
                d_pdb = self.identify_iCode_sequence(d_pdb, chain, res_no, iCode, res_name, d_seq)
    ...
    def identify_iCode_sequence(self, d_pdb, chain, res_no, iCode, res_name, d_seq):
        ...
        for iCode_prev in l_iCodes:
            ...
            if (
                ...
                ATOMseq,d_res_nos = self.ATOM2seq(d_pdb, chain, d_seq, res_no_max = res_no)
                res_symbol = self.res_name2res_symbol(res_name)
                ...
                ## e.g. 1nuo.pdb
                    ...
                    len(ATOMseq) > 0 and
                    res_symbol == d_seq['SEQRES'][chain]['seq'][len(ATOMseq)] and
                    ATOMseq == d_seq['SEQRES'][chain]['seq'][:len(ATOMseq)]
                    ):
                ...
                ## e.g. 2lve.pdb
                    ...
                    len(ATOMseq) > 0 and
                    res_symbol == d_seq['SEQRES'][chain]['seq'][len(ATOMseq)+index_alphabet] and
                    ATOMseq == d_seq['SEQRES'][chain]['seq'][:len(ATOMseq)] and
                ...
                ## e.g. 2j5q (chain B, res_no 54, iCode C)
                    ...
                    len(ATOMseq) > 0 and
                    res_symbol == d_seq['SEQRES'][chain]['seq'][len(ATOMseq)+index_alphabet-1] and
                    ATOMseq == d_seq['SEQRES'][chain]['seq'][:len(ATOMseq)] and
                    self.s_alphabet[index_alphabet-1] in l_iCodes and
                    ' ' not in l_iCodes 
                    ):
                ...
                ## e.g. 1uij.pdb
                    ...
                    len(ATOMseq) > 0 and
                    res_symbol == d_seq['SEQRES'][chain]['seq'][len(ATOMseq)+len(l_iCodes)] and
                    ATOMseq == d_seq['SEQRES'][chain]['seq'][:len(ATOMseq)]
                    ):
                    None
                else: ## e.g. 1fne
                    ...
                    print d_seq['SEQRES'][chain]['seq'][len(ATOMseq)+index_alphabet]
                    print
                    print len(ATOMseq) > 0
                    print res_symbol == d_seq['SEQRES'][chain]['seq'][len(ATOMseq)+index_alphabet-1]
                    print ATOMseq == d_seq['SEQRES'][chain]['seq'][:len(ATOMseq)]
                    print self.s_alphabet[index_alphabet-1] in l_iCodes
                    print 
                    print 1, ATOMseq, self.res_name2res_symbol(res_name)
                    print 2, d_seq['SEQRES'][chain]['seq']
    ...
    def spherermsd(
        ...
        for rep_chain1 in d_chains_interpdb_sequence_similar.keys():
            ...
            ATOMseq1,d_res_nos1 = self.ATOM2seq(d_pdb[pdb1], rep_chain1, d_seq[pdb1])
            ATOMseq2,d_res_nos2 = self.ATOM2seq(d_pdb[pdb2], rep_chain2, d_seq[pdb2])
