...
##        self.l_pdbs = ['2g34','2g33']
        self.l_pdbs = []
        self.l_pdbs = [
            '2g34','2g33',
            '1if0', '2fte', '2frp', '2fsy', '1ohg', '2ft1', '2gp1', '2fs3',
            '1mva', '1mvb', '2bny', '5msf', '2bu1', '2iz9', '2b2g', '2b2d', '2b2e', '1zdi', '1zdh', '1zdk', '1zdj', '2c4q', '1zse', '1kuo', '2c4z', '1aq3', '1aq4', '2c50', '2c51', '1bms', '6msf', '7msf', '2c4y', '1mst', '2ms2', '1u1y', '2bq5', '2iz8', '2bs1', '2bs0', '1msc', '2izn'
            ]
...
##        print 'calculating rmsd for chains %s, %s of %s, %s' %(chains1, chains2, pdb1, pdb2)
        ...
        for i in range(len(chains1)):
            ...
            for rep_chain1 in d_chains_intrapdb_sequence_identical[pdb1]:
                if chain1[0] == a:
                    a
...
##            for i in range(len(d_chains_interpdb_sequence_similar[rep_chain1])): ## e.g. 1k56.pdb,1k57.pdb
        ...
        d_biomolecules = {
            pdb1:{
                'biomolecule':biomolecule1,'l_chains':l_chains1,
                'rotation':{},'translation':{}
                },
            pdb2:{
                'biomolecule':biomolecule2,'l_chains':l_chains2,
                'rotation':{},'translation':{}
                },
            }

        ##
        ## apply REMARK350 transformations to l_coordinates and not to d_pdb to save memory when icosahedral structures
        ##
        for pdb in d_biomolecules.keys():
                ...
                for chain in chains:
                    if 'REMARK350' not in d_seq[pdb]:
                        ...
                        for matrix_no in matrix_nos:
                            ...
                            d_biomolecules[pdb]['rotation'][thcain] = rmatrix
                            d_biomolecules[pdb]['translation'][thcain] = tvector
                            if len(tchain) > 1:
                                if int(tchain[2:]) > 1:
...
##                                print 'rmsdchains2', rmsdchains2
                        ...
                        for k in range(len(tchains1)-j+1):
                            ...
                            if (
                                ...
                                ((j-1) % 60 != 0 and chains2[-1][0] != chain2[0]) ## disimilar chain letter
                                ):
                                rmsd = 'N/A'
                            else:
                                rmsd, n_chains, n_residues, n_coordinates, tv1, rm, tv2 = self.calculate_rmsd_for_multiple_chains(rmsdchains1,rmsdchains2,d_pdb,pdb1,pdb2,d_seq,d_chains_interpdb_sequence_similar,d_chains_intrapdb_sequence_identical,verbose=False)
                                rmsd = '%5.1f' %(rmsd)
                            if len(tchains1) >= 60:
                                print 'rmsd %s %s %4s %4s %5s %3s of %3s' %(pdb1, pdb2, tchains1[j-1], chain2, rmsd, j, len(tchains1))
...
##        print pdb, chain
    ...
    def ATOMrecords2coordinates(
        ...
        for SEQRESpos1 in range(l2,ATOMseq_len+l2):
...
##        print 'parsing coordinates of %s' %(s_pdb)
        ...
        for i in range(len(lines)):
            ...
            if record == 'ATOM':
                d_coordinates = self.parse_recordATOM(line, d_coordinates, lines, i, d_seq, s_pdb)[0]
