...
## what to do if MAN-->GLC and GLC<--MAN are considered different
...
##            ## pdb errors
            ...
            '1a4f', ## identical ATOM/HETATM IDs (HEM/OXY)
            '1rvw', ## identical ATOM/HETATM IDs (HOH/PO4)
            '11gs', ## identical ATOM/HETATM IDs (GTT/PRO)
            '2alu', ## identical ATOM/HETATM IDs ( FE/PHE A686 )
            '1nkx', ## identical ATOM/HETATM IDs ( FE/PHE A686 )
            '1a6g', ## identical ATOM/HETATM IDs (CMO/HEM  154 )
            '1a6m', ## identical ATOM/HETATM IDs (OXY/HEM  154 )
            '1mwe', ## identical ATOM/HETATM IDs (HOH/SIA    1 )
            '1np1', ## identical ATOM/HETATM IDs (HEM/LYS A  1 )
            '1aj9', ## identical ATOM/HETATM IDs (HOH/PO4    1 )
            '1hcn', ## identical ATOM/HETATM IDs (NAG/ASN A 52 )
            '1hvq', ## identical ATOM/HETATM IDs (HOH/NAG    1 )
            '1uyx', ## hydrogen bond between oxygen atoms
            '1mpm', ## oxygen-oxygen bond, incorrect?
            '1sbd','1sbe', ## oxygen-oxygen bond, incorrect!
##            '1azx', ## identical ATOM/HETATM IDs
##            '1bfn', ## identical ATOM/HETATM IDs
##            '1x0s', ## identical ATOM/HETATM IDs
##            '1gss','3gss', ## identical ATOM/HETATM IDs
            ## tommy errors
        ...
        for i in range(len(res_nos)):
            ...
            for i in range(len(d_pdb['chains'][chain]['residues'][res_no]['l_iCodes'])):
                ...
                if res_name not in self.l_modres+self.l_nucleotides+self.d_res.keys():
                    continue
                if d_pdb['chains'][chain]['residues'][res_no]['d_iCodes'][iCode]['record'] != 'ATOM':
                    continue
                            ...
                            for atom_no2 in d_CONECT[hetID1][chain1][res_no1][iCode1][atom_no1]:
                                ...
                                if atom_name1[0] == 'C':
                                    ...
                                    if  atom_name2[0] == 'O':
                                        ...
                                        if hetID2 != 'ASN':
                                            stop
                                    elif atom_name2[0] in ['N']: ## e.g. ACI,DIB
                                        bond = atom_name1[1:]+','+atom_name2[1:]
                                    else:
                                        stop
                                ## glycoside bond 2
                                    ...
                                    if  atom_name1[0] == 'O':
                                        ...
                                        if hetID1 != 'ASN':
                                            stop
                                    elif atom_name1[0] in ['N']: ## e.g. ACI,DIB
                                        bond = atom_name2[1:]+','+atom_name1[1:]
                                    else:
                                        print s_pdb, hetID1, chain1, res_no1, iCode1, atom_no1, atom_name1, atom_no2, atom_name2, hetID2
                                        stop
                                ## disulphide bond
                                    ...
                                    print s_pdb, atom_name1, atom_name2, hetID1, hetID2, atom_no1, atom_no2
                                    stop
                                ...
                                if atom_name1 != 'SG' and atom_name2 != 'SG':
                                    ...
                                    if not monomer2 in d_adjacency_forward.keys():
                                        d_adjacency_forward[monomer2] = {}
                                    if monomer1 in d_adjacency_forward[monomer2]: ## temp!!!
                                        if bond != d_adjacency_forward[monomer2][monomer1]: ## temp!!!
                                            notexpected ## temp!!!
                                    d_adjacency_forward[monomer2][monomer1] = bond
                                        
                                    if not monomer1 in d_adjacency_backward.keys():
                                        d_adjacency_backward[monomer1] = []
                                    if not monomer2 in d_adjacency_backward[monomer1]:
...
##                                print hetID1,chain1,res_no1,iCode1,atom_name1,hetID2,chain2,res_no2,iCode2,atom_name2,connection
        ...
        ## change adjacency dictionaries if incorrect oxygen assignment
        ## incorrect C --> O sequence is assumed when multiple endpoints at a monomer, which is not an amino acid root
        ## e.g. 1vbo.pdb,1vbp.pdb (double backward)
        ## d_adjacency_forward = {'M406 ': {'M407 ': '1,6', 'M405 ': '1,3'}, 'L406 ': {'L407 ': '1,6', 'L405 ': '1,3'}, 'O406 ': {'O405 ': '1,3', 'O407 ': '1,6'}, 'J406 ': {'J405 ': '1,3', 'J407 ': '1,6'}, 'K406 ': {'K407 ': '1,6', 'K405 ': '1,3'}, 'N406 ': {'N405 ': '1,3', 'N407 ': '1,6'}}
        ## d_adjacency_backward = {'L407 ': ['L406 '], 'M407 ': ['M406 '], 'N405 ': ['N406 '], 'O405 ': ['O406 '], 'K407 ': ['K406 '], 'K405 ': ['K406 '], 'M405 ': ['M406 '], 'J407 ': ['J406 '], 'O407 ': ['O406 '], 'J405 ': ['J406 '], 'L405 ': ['L406 '], 'N407 ': ['N406 ']}
        for monomer2 in d_adjacency_forward.keys():
            monomers1 = d_adjacency_forward[monomer2].keys()
            ## continue if only one backward monomer
            if len(monomers1) == 0:
                print monomer2
                notexpected3
            if len(monomers1) == 1:
                continue
            if monomer2 not in d_adjacency_backward.keys():
                hetID = self.monomertranslation(monomer2,d_coordinates)
                if hetID in self.d_res.keys():
                    continue
                monomer1 = min(monomers1)
                bond = d_adjacency_forward[monomer2][monomer1]
                if monomer1 > monomer2:
                    print monomer1, monomer2
                    notexpected1
                d_adjacency_forward[monomer1] = {monomer2:bond}
                del d_adjacency_forward[monomer2][monomer1]
                if monomer2 in d_adjacency_backward.keys():
                    print d_adjacency_backward
                    notexpected2
                d_adjacency_backward[monomer2] = monomer1
                del d_adjacency_backward[monomer1]
        ## e.g. 1ona.pdb (double forward)
        ## d_adjacency_forward = {'A240 ': {'A241 ': '6,1'}, 'D240 ': {'D241 ': '6,1'}, 'D242 ': {'D241 ': '3,1'}, 'A242 ': {'A241 ': '3,1'}, 'C242 ': {'C241 ': '3,1'}, 'C240 ': {'C241 ': '6,1'}, 'B242 ': {'B241 ': '3,1'}, 'B240 ': {'B241 ': '6,1'}}
        ## d_adjacency_backward = {'A241 ': ['A240 ', 'A242 '], 'D241 ': ['D242 ', 'D240 '], 'B241 ': ['B242 ', 'B240 '], 'C241 ': ['C240 ', 'C242 ']}
        for monomer1 in d_adjacency_backward.keys():
            monomers2 = d_adjacency_backward[monomer1].keys()
            print monomer1, monomers2
            stop
...
##                    print 'b0path', path, monomer1, l_branches
    ...
    def parse_recordCONECT(self,line,d_atomnos,d_CONECT):
        ...
        if res_name1 in self.l_cofactors+self.l_solvents:
            return d_CONECT
        ...
        for atom_no2 in atom_nos[1:]:
            ...
            if atom_name2[0] == 'H':
                continue
            ## skip if a cofactor to which molecules are not *covalently* bound
            if res_name2 in self.l_cofactors+self.l_solvents:
                continue
            if (
                chain1 !=  chain2 or
                res_no1 != res_no2 or
...
##                        transformation = True
    ...
    def parse_recordREMARK465(self, line, d_pdb, lines, i):
        ...
        if line[10:].strip() == 'M RES C SSSEQI':
            ...
            for j in range(i+1,len(lines)):
                ...
                if res_name in self.d_res.keys():
                    record = 'ATOM'
                elif res_name in self.l_modres:
                    record = 'HETATM'
                else:
                    print res_name
                    notexpected
    ...
    def parse_recordATOM(self, line, d_pdb, lines, i, d_seq, s_pdb, record):
        ...
        ## temp!!!
            ...
            print s_pdb, chain, res_no, iCode ## temp!!!
            stop
        ## check that res_name is correct (e.g. 2fes:L:1)
        elif d_pdb['chains'][chain]['residues'][res_no]['d_iCodes'][iCode]['res_name'] != res_name and atom_altloc == ' ': ## 1fh2:A:30 atom_altloc
            ## change the iCode
