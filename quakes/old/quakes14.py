...
## clusters95
        ...
        for cluster in l_clusters:
            if cluster not in [4722,10083,11201]:
                continue
            if cluster < 130:
                continue
    ...
    def analyze_sequences(self, d_seq, d_chains_intrapdb_sequence_identical):
        ...
        for i1 in range(self.pdbcount-1):
            ...
            for i2 in range(i1+1,self.pdbcount):
                ...
                if (
                    (self.pdb1 == '2frp' and self.pdb2 == '2gp1') or
                    (self.pdb1 == '2gp1' and self.pdb2 == '2frp') or
                    (self.pdb1 == '2gp1' and self.pdb2 == '2fs3') or
                    (self.pdb1 == '2fs3' and self.pdb2 == '2gp1')
                    ): ## temp!!!
                    continue
                ...
                for biomolecule1 in d_biomolecules1.keys():
                    ...
                    for biomolecule2 in d_biomolecules2.keys():
                        ...
                        if d_hetero[pdb1] != d_hetero[pdb2]:
                            if len(d_hetero[pdb1]) != len(d_hetero[pdb2]):
                                print pdb1, pdb2
                                expected
                            fd = open('checkheterotitle','a')
...
##        print 'identifying different long peptides'
        ...
        for rep_chain1a in d_chains_interpdb_sequence_similar.keys():
            ...
            rep_chain2a = d_chains_interpdb_sequence_similar[rep_chain1a]['rep_chain2']
            for rep_chain1b in d_chains_interpdb_sequence_similar.keys():
                if rep_chain1a == rep_chain1b:
                    continue
                rep_chain2b = d_chains_interpdb_sequence_similar[rep_chain1b]['rep_chain2']
                if rep_chain2a == rep_chain2b:
                    SEQRESchains1_similar_to_SEQRESchains2 |= set([rep_chain1b])
                    SEQRESchains1_similar_to_SEQRESchains2 |= set(d_chains_intrapdb_sequence_identical[pdb1][rep_chain1b])
            SEQRESchains1_similar_to_SEQRESchains2 |= set([rep_chain1a])
            SEQRESchains1_similar_to_SEQRESchains2 |= set(d_chains_intrapdb_sequence_identical[pdb1][rep_chain1a])
        ...
        for rep_chain1a in d_chains_interpdb_sequence_similar.keys():
            ...
            for rep_chain1b in d_chains_interpdb_sequence_similar.keys():
                if rep_chain1a == rep_chain1b:
                    continue
                rep_chain2b = d_chains_interpdb_sequence_similar[rep_chain1b]['rep_chain2']
                print rep_chain1a, rep_chain1b, rep_chain2a, rep_chain2b
                if rep_chain2a == rep_chain2b:
                    SEQRESchains2_similar_to_SEQRESchains1 |= set([rep_chain2b])
                    SEQRESchains2_similar_to_SEQRESchains1 |= set(d_chains_intrapdb_sequence_identical[pdb2][rep_chain2b])
            SEQRESchains2_similar_to_SEQRESchains1 |= set(rep_chain2a)
            SEQRESchains2_similar_to_SEQRESchains1 |= set(d_chains_intrapdb_sequence_identical[pdb2][rep_chain2a])
            print rep_chain1a, rep_chain2a, SEQRESchains2_similar_to_SEQRESchains1
        ...
        print bmSEQRESchains1_similar_to_SEQRESchains2
        print bmSEQRESchains2_similar_to_SEQRESchains1
        print SEQRESchains1_not_similar_to_SEQRESchains2
        print SEQRESchains2_not_similar_to_SEQRESchains1
        stop
...
##        print 'identifying structurally equivalent long peptides by structure between pdbs'
        ...
        for rep_chain1 in d_chains_interpdb_sequence_similar.keys():
            rep_chain2 = d_chains_interpdb_sequence_similar[rep_chain1]['rep_chain2']
            l1SEQRES = d_chains_interpdb_sequence_similar[rep_chain1]['l1']
            l2SEQRES = d_chains_interpdb_sequence_similar[rep_chain1]['l2']
            ...
            if ATOMseq1 != d_chains_interpdb_sequence_similar[rep_chain1]['s1']:
                expected_not_critical
            if ATOMseq2 != d_chains_interpdb_sequence_similar[rep_chain1]['s2']:
                expected_not_critical
            d_chains_interpdb_sequence_similar[rep_chain1]['d_res_nos1'] = d_res_nos1
            d_chains_interpdb_sequence_similar[rep_chain1]['d_res_nos2'] = d_res_nos1
            d_chains_interpdb_sequence_similar[rep_chain1]['l1ATOM'] = l1
            d_chains_interpdb_sequence_similar[rep_chain1]['l2ATOM'] = l2
            d_chains_interpdb_sequence_similar[rep_chain1]['seqATOM1'] = l1
            d_chains_interpdb_sequence_similar[rep_chain1]['seqATOM2'] = l2
        ...
        for rep_chain1a in d_chains_interpdb_sequence_similar.keys():
            rep_chains1b = []
            if rep_chain1a in chains1:
                continue
            ## identify rep chain2 seq sim to rep chain1
            rep_chain2a = d_chains_interpdb_sequence_similar[rep_chain1a]['rep_chain2']
            if rep_chain2a in chains2:
                print pdb1, pdb2
                notexpected
            ## identify other rep chains1 with seq sim to rep chain2 ## e.g. 1nvv.pdb,1nvw.pdb (Q;R;S == Q,R;S)
            for rep_chain1b in d_chains_interpdb_sequence_similar.keys():
                if rep_chain1a == rep_chain1b:
                    continue
                rep_chain2b = d_chains_interpdb_sequence_similar[rep_chain1b]['rep_chain2']
                if rep_chain2a == rep_chain2b:
                    if rep_chain1b in bmchains1:
                        rep_chains1b += [rep_chain1b]
            ## identify chains identical to the representative chains
            chains2seqid = set([rep_chain2a]+d_chains_intrapdb_sequence_identical[pdb2][rep_chain2a])
            ...
            bmchains1seqid.sort()
            bmchains2seqid.sort()
            ## append rep_chains1b (chains sequence similar to rep_chain1a) to bmchains
            for rep_chain1b in rep_chains1b:
                chains1seqid = set(d_chains_intrapdb_sequence_identical[pdb1][rep_chain1b]+[rep_chain1b])
                bmchains1seqid += list(set(chains1seqid) & set(bmchains1))
...
##                    continue
        ...
        print d_chains_intrapdb_sequence_identical[pdb1].keys()
        print d_chains_intrapdb_sequence_identical[pdb2].keys()
                            ...
                            if rmsd <= self.maxrmsd:
                                ...
                                if len(tchains1) >= 60  and len(ksort) < 60:
                                    if len(chain2) > 1:
                                        ...
                                        ksort += [1]
                                break
                            if rmsd >= self.maxrmsd and j == 1 and k == 0 and len(tchains1) >= 60: ## temp!!!
                                fd = open('checkvirus.txt','a')
                                fd.write('%s %s\n' %(pdb1,pdb2))
                                fd.close()
...
##        print pdb, chain
    ...
    def alignATOMseq(self, d_pdb, d_seq, pdb1, pdb2, chain1, chain2):
        ...
        ##
        if len(ATOMseq1) == len(ATOMseq2) and ATOMseq1 == ATOMseq2:
        ...
        for i in range(len(lines)):
            ...
            if record == 'ATOM':
                ...
                if res_name in self.d_modres.keys(): ## e.g. 1gcj.pdb
                    d_coordinates, d_line = self.parse_recordATOM(line, d_coordinates, lines, i, d_seq, s_pdb)
                elif res_name in ['HOH','H20','DOD','D2O']:
                    continue
                else:
                    d_coordinates, d_line = self.parse_recordATOM(line, d_coordinates, lines, i, d_seq, s_pdb, appendtodictionary=False)
                atom_no = d_line['atom_no']
                d_HETATMatomno[atom_no] = d_line
                res_no = d_line['res_no']
                if res_no in d_HETATMresno.keys(): ## temp!!!
                    if d_line['res_name'] != d_HETATMresno[res_no]['res_name']:
                        print line
                        print s_pdb
                        notexpected
                if res_no in d_HETATMresno.keys(): ## temp!!!
                    if d_line['chain'] != d_HETATMresno[res_no]['chain']:
                        print line
                        print s_pdb
                        notexpected
                d_HETATMresno[res_no] = d_line
                ...
                for j in range(6,31,5):
                    if line[j:j+5] != '     ':
                        atom_nos += [line[j:j+5].strip()]
                atom_nos2 = line[6:].split()
                if atom_nos != atom_nos2:
                    print atom_nos, atom_nos2
                    expected
                res_nos = set()
                for atom_no in atom_nos:
                    atom_no = int(atom_no)
                    if atom_no in d_HETATMatomno.keys():
                        res_no = d_HETATMatomno[atom_no]['res_no']
                        res_nos |= set([res_no])
                if len(res_nos) > 1:
                    appended = False
                    for j in range(len(l_connections)):
                        connection = l_connections[j]
                        if len(connection & res_nos) > 0:
                            l_connections[j] |= res_nos
                            appended = True
                    if appended == False:
                        l_connections += [res_nos]
...
##                d_hetero['chains'][chain]['residues'][res_no]['iCodes'][iCode] = hetID
        ...
        for j in range(len(l_connections)):
            ...
            l_hetIDs = []
            chains = set([]) ## temp!!!
            for res_no in connections:
                res_name = d_HETATMresno[res_no]['res_name']
                chains |= set([d_HETATMresno[res_no]['chain']]) ## temp!!!
                l_hetIDs += [res_name]
            if len(chains) != 1:
                print s_pdb
                notexpected
...
##        set_hetIDs = set([])
##        res_nos = list(set(d_HETATMresno.keys())-set_connections)
##        for res_no in res_nos:
##            set_hetIDs |= set([d_HETATMresno[res_no]['res_name']])
##        print set_hetIDs
##        stop
...
##                        transformation = True
    ...
    def parse_recordATOM(self, line, d_pdb, lines, i, d_seq, s_pdb, appendtodictionary = True):
