...
class quakes:
    ...
    def main(self):
        ...
        pdbs += ['1a50.pdb','1a5s.pdb','1a2l.pdb','1a2m.pdb','1fz3.pdb','1fz1.pdb','1hn4.pdb','1fx9.pdb','1dlf.pdb','2dlf.pdb','1q1r.pdb','1q1w.pdb','1csg.pdb','2gmf.pdb','1fia.pdb','3fis.pdb','1fph.pdb','1hao.pdb','1hap.pdb','1nrr.pdb']
...
##        print 'inter', d_chains_interpdb_sequence_similar
    ...
    def ATOMi2seq(self, d_pdb, pdb, chain, res_no):
    ...
    def ATOM2seq(self, d_pdb, pdb, chain):
...
##will not work if modified residues..? skip MODRES..?
            ## append res to seq if more than just an ins is associated with the res no
            if 'insertion' in d_pdb[pdb]['chains'][chain]['residues'][res_no].keys():
                if d_pdb[pdb]['chains'][chain]['residues'][res_no]['insertion']['position'] == 'leading':
                    seq += self.ATOMi2seq(d_pdb, pdb, chain, res_no)
            if 'res_name' in d_pdb[pdb]['chains'][chain]['residues'][res_no].keys():
                ...
                if res_name in self.d_res.keys():
                    ...
                    seq += 'X'
            if 'insertion' in d_pdb[pdb]['chains'][chain]['residues'][res_no].keys():
                if d_pdb[pdb]['chains'][chain]['residues'][res_no]['insertion']['position'] == 'trailing':
                    seq += self.ATOMi2seq(d_pdb, pdb, chain, res_no)
    ...
    def dcoordinates2lcoordinates(self, d_pdb, d_seq, pdb1, pdb2, chain1, chain2):
        ...
        SEQRESseq2 = d_seq[pdb2]['chains'][chain2]['seq']

        ##
        ## parse residue sequences from ATOM records
        ## 
        d_ATOM_seqs = {
            pdb1:{'chain':chain1,'SEQRESseq':SEQRESseq1},
            pdb2:{'chain':chain2,'SEQRESseq':SEQRESseq2},
            }
        for pdb in d_ATOM_seqs.keys():
            chain = d_ATOM_seqs[pdb]['chain']
            SEQRESseq = d_ATOM_seqs[pdb]['SEQRESseq']
            seq = self.ATOM2seq(d_pdb, pdb, chain, SEQRESseq)
...
##        ##
##        d_seqs = {
##            pdb1:{'ATOMseq':ATOMseq1,'SEQRESseq':SEQRESseq1},
##            pdb2:{'ATOMseq':ATOMseq2,'SEQRESseq':SEQRESseq2}
##            }
...
##                    res_no_diff_numbering -= count_missingresidues
        ...
        ## necessary to align sequences if REMARK 465 not specified for missing residues (e.g. residues 1-7 of chain D of 1RAI vs 1RAF)
        ##
        if ATOMseq1 != ATOMseq2:
            ...
            r2 = len(s2)-len(s2.rstrip('-'))
            r = max(r1,r2)
            ## the four lines below will not take into consideration terminal insertions...!!!
            if r2 == 0: resnos1 = resnos1[l2:]
            else: resnos1 = resnos1[l2:-r2]
            if r1 == 0: resnos2 = resnos2[l1:]
            else: resnos2 = resnos2[l1:-r1]
...
##            resrange = len(ATOMseq1)
        ...
        i2 = 0
        rescount = 0
        while i1 < len(resnos1) and i2 < len(resnos2):
##            print pdb1, pdb2, i1, i2

            res_no1 = resnos1[i1]
            res_no2 = resnos2[i2]
##            if next_iCode2 == '' and next_iCode1 == '':
##                res_no2 = res_no1+res_no_diff_numbering+seq1insertions-seq2insertions
##                res_no2 = resnos2[i+seq1insertions-seq2insertions]
##            elif next_iCode2 != '' and next_iCode1 == '':
##                res_no2 = resnos1[i-1]+res_no_diff_numbering+seq1insertions-seq2insertions
##                res_no2 = resnos2[i+seq1insertions-seq2insertions]
##            elif next_iCode2 == '' and next_iCode1 != '':
##                res_no2 = resnos1[i+1]+res_no_diff_numbering+seq1insertions-seq2insertions
##                res_no2 = resnos2[i+seq1insertions-seq2insertions]
...
####                    fd.write("                'REMARK 465     %3s %1s  %4i', ## %s.pdb %s.pdb\n" %(res_name, chain, res_no2, pdb2, pdb1))
####                    fd.close()
##                    ## not addition if insertion...
##                    i1 += 1
##                    i2 += 1
...
##            if not res_no1 in d_pdb[pdb1]['chains'][chain1]['residues'].keys():
##                continue
##            if not res_no2 in d_pdb[pdb2]['chains'][chain2]['residues'].keys():
##                continue

            if 'REMARK' in d_pdb[pdb1]['chains'][chain1]['residues'][res_no1].keys():
                i1 += 1
                if next_iCode2 == '': ## temporary incomplete solution
                    i2 += 1
                continue
            if 'REMARK' in d_pdb[pdb2]['chains'][chain2]['residues'][res_no2].keys():
                i2 += 1
                if next_iCode1 == '': ## temporary incomplete solution
                    i1 += 1
                continue

            d_resname = {
                pdb1:{'chain':chain1,'res_no':res_no1,'next_iCode':next_iCode1,'seqinsertions':seq1insertions},
                pdb2:{'chain':chain2,'res_no':res_no2,'next_iCode':next_iCode2,'seqinsertions':seq2insertions},
                }
            for pdb in d_resname.keys():
                chain = d_resname[pdb]['chain']
                res_no = d_resname[pdb]['res_no']
                next_iCode = d_resname[pdb]['next_iCode']
                seqinsertions = d_resname[pdb]['seqinsertions']
                if 'insertion' in d_pdb[pdb]['chains'][chain]['residues'][res_no].keys():
                    ## first instance of res_no, trailing insertion
                    if (
                        next_iCode in ['','N/A'] and
                        d_pdb[pdb]['chains'][chain]['residues'][res_no]['insertion']['position'] == 'trailing'
                        ):
                        res_name = d_pdb[pdb]['chains'][chain]['residues'][res_no]['res_name']
                        d_atoms = d_pdb[pdb]['chains'][chain]['residues'][res_no]['atoms']
                        next_iCode = d_pdb[pdb]['chains'][chain]['residues'][res_no]['insertion']['l_iCodes'][0]
                        print 'a', pdb, chain, res_no, res_name, '"'+next_iCode+'"'#
                    ## first instance of res_no, leading insertion
                    elif (
                        next_iCode in ['','N/A'] and
                        d_pdb[pdb]['chains'][chain]['residues'][res_no]['insertion']['position'] == 'leading'
                        ):
                        iCode = d_pdb[pdb]['chains'][chain]['residues'][res_no]['insertion']['l_iCodes'][0]
                        res_name = d_pdb[pdb]['chains'][chain]['residues'][res_no]['insertion']['d_iCodes'][iCode]['res_name']
                        d_atoms = d_pdb[pdb]['chains'][chain]['residues'][res_no]['insertion']['d_iCodes'][iCode]['atoms']
                        if len(d_pdb[pdb]['chains'][chain]['residues'][res_no]['insertion']['l_iCodes']) > 1:
                            next_iCode = d_pdb[pdb]['chains'][chain]['residues'][res_no]['insertion']['l_iCodes'][1]
                        else:
                            if 'res_name' in d_pdb[pdb]['chains'][chain]['residues'][res_no].keys():
                                next_iCode = ' '
                            else:
                                next_iCode = ''
                        print 'b', pdb, chain, res_no, res_name, '"'+next_iCode+'"'#
                    ## neither first nor last instance of ires_no
                    elif (
                        next_iCode not in ['','N/A',' '] and
                        next_iCode != d_pdb[pdb]['chains'][chain]['residues'][res_no]['insertion']['l_iCodes'][-1]
                        ):
                        seqinsertions += 1
                        res_name = d_pdb[pdb]['chains'][chain]['residues'][res_no]['insertion']['d_iCodes'][next_iCode]['res_name']
                        d_atoms = d_pdb[pdb]['chains'][chain]['residues'][res_no]['insertion']['d_iCodes'][next_iCode]['atoms']
                        index = d_pdb[pdb]['chains'][chain]['residues'][res_no]['insertion']['l_iCodes'].index(next_iCode)
                        next_iCode = d_pdb[pdb]['chains'][chain]['residues'][res_no]['insertion']['l_iCodes'][index+1]
                        print 'c', pdb, chain, res_no, res_name, '"'+next_iCode+'"'#
                    ## last instance of ires_no, leading and trailing insertion
                    elif (
                        next_iCode not in ['','N/A',' '] and
                        next_iCode == d_pdb[pdb]['chains'][chain]['residues'][res_no]['insertion']['l_iCodes'][-1]
                        ):
                        ...
                        ## leading
                        if (
                            'res_name' in d_pdb[pdb]['chains'][chain]['residues'][res_no].keys() and
                            d_pdb[pdb]['chains'][chain]['residues'][res_no]['insertion'] == 'leading'
                            ):
                            next_iCode = ' '
                        ## trailing or last instance of res_no
                        else:
                            next_iCode = ''
                        print 'd', pdb, chain, res_no, res_name, '"'+next_iCode+'"'#
                    ## last instance of res_no, leading insertion
                    elif next_iCode == ' ':
                        res_name = d_pdb[pdb]['chains'][chain]['residues'][res_no]['res_name']
                        d_atoms = d_pdb[pdb]['chains'][chain]['residues'][res_no]['atoms']
                        next_iCode = ''
                        print 'e', pdb, chain, res_no, res_name, '"'+next_iCode+'"'#
                    else:
                        stop
                else:
                    res_name = d_pdb[pdb]['chains'][chain]['residues'][res_no]['res_name']
                ...
                d_resname[pdb]['next_iCode'] = next_iCode
                d_resname[pdb]['seqinsertions'] = seqinsertions
                d_resname[pdb]['d_atoms'] = d_atoms
            res_name1 = d_resname[pdb1]['res_name']
            next_iCode1 = d_resname[pdb1]['next_iCode']
            seq1insertions = d_resname[pdb1]['seqinsertions']
            res_name2 = d_resname[pdb2]['res_name']
            next_iCode2 = d_resname[pdb2]['next_iCode']
            seq2insertions = d_resname[pdb2]['seqinsertions']
            d_atoms1 = d_resname[pdb1]['d_atoms']
            d_atoms2 = d_resname[pdb2]['d_atoms']
            print pdb1, pdb2, chain1, chain2, res_no1, res_no2, res_name1, res_name2, seq1insertions, seq2insertions

            if res_name1 != res_name2:
                ## continue if residue missing
                resnogap1 = resnos1[i1]-resnos1[i1-1]
                resnogap2 = resnos2[i2]-resnos2[i2-1]
                if resnogap1 > 1 and resnogap2 == 1:
                    i2 += 1
                    next_iCode2 = ''
                    continue
                elif resnogap2 > 1 and resnogap1 == 1:
                    i1 += 1
                    next_iCode1 = ''
                    continue
                elif resnogap1 > 1 and resnogap2 > 1 and res_no1 > res_no2+res_no_diff_numbering:
                    i2 += 1
                    next_iCode2 = ''
                    continue
                elif resnogap1 > 1 and resnogap2 > 1 and res_no1 < res_no2+res_no_diff_numbering:
                ...
                print res_no_diff_numbering, l1, l2, r1, r2
...
##                fd.close()
        ...
        if (
            (
                res_name1 != d_pdb[pdb1]['chains'][chain1]['residues'][maxresno1]['res_name']
                or
                res_name2 != d_pdb[pdb2]['chains'][chain2]['residues'][maxresno2]['res_name']
                )
            and
            (
                'REMARK' not in d_pdb[pdb1]['chains'][chain1]['residues'][maxresno1].keys()
                and
                'REMARK' not in d_pdb[pdb2]['chains'][chain2]['residues'][maxresno2].keys()
                )
            ):
            print res_name1, res_name2
            print res_no1, res_no2
            print maxresno1, maxresno2
            print pdb1, pdb2
            stop ## temporary
...
##                    continue
            ...
            ## only parse ATOM records to get information about insertions in SEQRES records
...
##        print 'parsing coordinates of %s' %(s_pdb)
    ...
    ##    atom_altloc = line[16].strip()
        ...
        if iCode == ' ':
            ...
            if not 'res_names' in d_pdb[s_pdb]['chains'][chain]['residues'][res_no]['insertion'].keys():
                d_pdb[s_pdb]['chains'][chain]['residues'][res_no]['insertion']['res_names'] = []
            ...
            ## res_no > position
            if not 'position' in d_pdb[s_pdb]['chains'][chain]['residues'][res_no]['insertion'].keys():
                d_pdb[s_pdb]['chains'][chain]['residues'][res_no]['insertion']['position'] = position
