...
##        d_data[pdb], lines = calculate_phipsi(pdb)
        if pdb == '150l':
            chain = 'D'
        else:
            chain = 'A'
        d_data[pdb], lines = calculate_phipsi(pdb, chain = chain)
    ...
    pdb2 = l_pdbs[1]
    l3 = []
    for res_no in range(0+1+1,min(max(d_data[pdb1].keys()),max(d_data[pdb2].keys()),)+1-1+1):
        phi1 = d_data[pdb1][res_no][0]
        psi1 = d_data[pdb1][res_no][1]
        phi2 = d_data[pdb2][res_no][0]
        psi2 = d_data[pdb2][res_no][1]
        omega1 = d_data[pdb1][res_no][2]
        omega2 = d_data[pdb2][res_no][2]
        if phi1 < -90 and phi2 > 90:
            phi1 += 360
        if psi1 < -90 and psi2 > 90:
            psi1 += 360
        if phi1 > 90 and phi2 < -90:
            phi2 += 360
        if psi1 > 90 and psi2 < -90:
            psi2 += 360
        delta = math.sqrt((phi2-phi1)**2+(psi2-psi1)**2)
        if delta > 1300:
            print res_no,phi1,psi1,phi2,psi2
            print delta
            stop
        l2 += [delta]
        l += [[delta,res_no]]
...
def calculate_pseudo(pdb, chain = None):
...
##        if res_no not in res_range:
##            continue
        if res_no-2 in d_coordinates[chain].keys() and res_no-1 in d_coordinates[chain].keys() and res_no+1 in d_coordinates[chain].keys() and res_no+2 in d_coordinates[chain].keys():
            CA_prev2 = d_coordinates[chain][res_no-2]['atoms']['CA']
            CA_prev1 = d_coordinates[chain][res_no-1]['atoms']['CA']
            CA = d_coordinates[chain][res_no]['atoms']['CA']
            CA_next1 = d_coordinates[chain][res_no+1]['atoms']['CA']
            CA_next2 = d_coordinates[chain][res_no+2]['atoms']['CA']
            phi = dihedral(CA_prev2,CA_prev1,CA,CA_next1,)
            psi = dihedral(CA_prev1,CA,CA_next1,CA_next2,)
            res_name = ''
...
def calculate_phipsi(pdb, chain = None):
...
##            continue
        ...
        if res_no-1 in d_coordinates[chain].keys() and res_no+1 in d_coordinates[chain].keys():
            ...
            CA_next = d_coordinates[chain][res_no+1]['atoms']['CA']
            omega = dihedral(CA,C,N_next,CA_next)
            s = '%s %s %s%s\n' %(phi,psi,res_name,res_no)
            lines += [s]

            d_data[res_no] = [phi,psi,omega,]
